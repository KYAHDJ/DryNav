...
class AuthRepository @Inject constructor(
    ...
    /**
     * Waits until Firebase has a usable authenticated user. The app starts
     * anonymous authentication in DryNavApplication, but that sign-in is
     * asynchronous. A report can therefore be submitted during the small
     * window where currentUser is still null, which makes Firestore/Storage
     * correctly reject the request with PERMISSION_DENIED.
     *
     * If the user is already signed in (email/Google), this returns that
     * account unchanged. Otherwise it waits for/creates the anonymous session.
     */
    suspend fun ensureAuthenticated(): FirebaseUser = auth.currentUser
        ?: auth.signInAnonymously().await().user
        ?: error("Firebase authentication did not return a user")
    ...
    suspend fun updateProfile(
        ...
        val user = currentUser ?: error("No signed-in user")
        user.updateProfile(userProfileChangeRequest {
            this.displayName = displayName.trim()
            this.photoUri = photoUrl?.let(android.net.Uri::parse)
        }).await()
        ...
        // Keep existing flood markers in sync with the user's current public
        // identity so the map avatar changes everywhere after Profile Save.
        val reports = firestore.collection("flood_reports")
            .whereEqualTo("reporterId", user.uid)
            .get().await()
        reports.documents.chunked(400).forEach { chunk ->
            val batch = firestore.batch()
            chunk.forEach { doc ->
                batch.update(
                    doc.reference,
                    mapOf(
                        "reporterName" to displayName.trim(),
                        "reporterPhotoUrl" to (photoUrl ?: "")
                    )
                )
            }
            batch.commit().await()
