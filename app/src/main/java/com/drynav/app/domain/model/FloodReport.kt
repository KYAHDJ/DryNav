...
data class FloodReport(
    ...
    /** Timestamp and GPS captured at the moment each report photo was taken. */
    val photoMetadata: List<PhotoMetadata> = emptyList(),
    /**
     ...
     * (best-effort — blank if the lookup failed).
    ...
    /** Approximate radius of the pinned flood area, in meters. */
    val floodRadiusMeters: Double = 40.0,
    /** Firebase user who created the report. */
    val reporterId: String = "",
    /** Public display name cached on the report for map attribution. */
    val reporterName: String = "",
    /** Public profile photo URL cached on the report for map attribution. */
    ...
    fun toFirestoreMap(): Map<String, Any> = mapOf(
        ...
        "photoMetadata" to photoMetadata.map {
            mapOf(
                "capturedAt" to it.capturedAt,
        ...
        "floodRadiusMeters" to floodRadiusMeters,
        "reporterId" to reporterId,
        "reporterName" to reporterName,
        "reporterPhotoUrl" to reporterPhotoUrl,
        "expiresAt" to timestamp + REPORT_LIFETIME_MS
    ) + buildMap {
    ...
    companion object {
        ...
        fun fromFirestore(id: String, data: Map<String, Any?>): FloodReport = FloodReport(
            ...
            photoUrls = (data["photoUrls"] as? List<*>)?.filterIsInstance<String>()
                ...
                ?: emptyList(),
            photoMetadata = parsePhotoMetadata(data),
            ...
            floodRadiusMeters = (data["floodRadiusMeters"] as? Number)?.toDouble() ?: 40.0,
            reporterId = (data["reporterId"] as? String).orEmpty(),
            reporterName = (data["reporterName"] as? String).orEmpty(),
            reporterPhotoUrl = (data["reporterPhotoUrl"] as? String).orEmpty()
        )

        private fun parsePhotoMetadata(data: Map<String, Any?>): List<PhotoMetadata> =
            (data["photoMetadata"] as? List<*>).orEmpty().mapNotNull { raw ->
                val map = raw as? Map<*, *> ?: return@mapNotNull null
                val capturedAt = (map["capturedAt"] as? Number)?.toLong() ?: return@mapNotNull null
                ...
                PhotoMetadata(capturedAt, latitude, longitude)
            }
...
/** Metadata captured at the device camera capture event. */
