...
fun MapScreen(
    ...
    val floodPinIcon = remember { createPinBitmap(AndroidColor.rgb(229, 57, 53)) }
    ...
    LaunchedEffect(
        ...
        uiState.floodReports.forEach { report ->
            annotationManager.create(
                PointAnnotationOptions()
                    .withPoint(Point.fromLngLat(report.longitude, report.latitude))
                    .withIconImage(floodPinIcon)
                    .withIconAnchor(IconAnchor.BOTTOM)
                    .withIconSize(0.38)
            )
        }
    ...
    // Public map flood zones gently pulse so they remain visible without
    // changing the report-placement preview. The animation only updates
    // the flood overlay geometry; navigation, routes and pins are untouched.
    LaunchedEffect(uiState.floodReports, uiState.floodOverlayEnabled) {
        var phase = 0.0
        while (isActive) {
            mapView.getMapboxMap().getStyle()?.let { style ->
                val pulse = 1.0 + kotlin.math.sin(phase) * 0.10
                updateFloodHeatmap(style, uiState.floodReports, uiState.floodOverlayEnabled, pulseScale = pulse)
            }
            phase += 0.16
            delay(70)
        }
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbarHostState) }) { padding ->
        Box(
            ...
            uiState.selectedFlood?.let { flood ->
                Surface(
