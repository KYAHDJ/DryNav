...
fun MapHomeScreen(
    ...
    // Public map flood zones gently pulse so the affected areas stand out
    // from the base map. This animation is intentionally absent from the
    // report-placement screen.
    LaunchedEffect(uiState.floodReports, uiState.floodOverlayEnabled) {
        var phase = 0.0
        while (isActive) {
            mapView.getMapboxMap().getStyle()?.let { style ->
