...
fun TutorialOverlay(manager: TutorialManager) {
    ...
    BoxWithConstraints(
        ...
        val isFullPageTarget = bounds != null &&
            bounds.top <= padPx && bounds.bottom >= screenHeightPx - padPx
        val isProfilePageTarget = step.targetKey == "profile_page"
        val showBelow = bounds == null || spaceBelowPx >= spaceAbovePx

        // The Profile tutorial intentionally spotlights the entire page.
        // Unlike a small button/card target, there may be almost no free space
        // below the target for the description card. Keep the card in a stable
        // bottom position so the tutorial text can never be clipped or pushed
        // off-screen.
        val regionModifier = with(density) {
            if (isProfilePageTarget || isFullPageTarget) {
                Modifier
        ...
        Box(
            modifier = if (isProfilePageTarget || isFullPageTarget) {
                regionModifier
                    .padding(horizontal = 24.dp, vertical = 14.dp)
                    .navigationBarsPadding()
            } else {
                regionModifier.fillMaxWidth()
            },
            contentAlignment = if (isProfilePageTarget || isFullPageTarget) {
